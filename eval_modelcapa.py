...
def run(x: torch.Tensor, y: torch.Tensor, x_test: torch.Tensor,
    ...
    for attack in range(len(args.attacks)):
        for eps in range(len(args.epss)):
            adv_loss, adv_acc, adv_bca = trainer.robust_test(test_loader,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='DeepCNN') #ShallowCNN DeepCNN EEGNet
    parser.add_argument('--dataset', type=str, default='weibo')
    parser.add_argument('--cross_balance', type=int, default=0)


    parser.add_argument('--midDim', type=int, default=40)
    parser.add_argument('--d1', type=int, default=1)
    parser.add_argument('--d2', type=int, default=2)
    parser.add_argument('--d3', type=int, default=2)
    
    parser.add_argument('--attacks', nargs='+', help='<Required> Set flag',default=[])#['FGSM', 'PGD','autoPGD']
    parser.add_argument('--epss', nargs='+', help='<Required> Set flag',type=float,default=[ 0.005, 0.01,0.015,0.02,0.025,0.03,0.035,0.04,0.045,0.05])#[0.01,0.03,0.05]


    parser.add_argument('--setup', type=str, default='within')
    parser.add_argument('--ea', type=str, default='sub') # no sub sess


    parser.add_argument('--train', nargs='+',default=['NT','ATchastd','ATchastd','ATchastd','ATchastd','ATchastd'])#['NT', 'ATchastd_fgsm', 'ATchastd_fgsm', 'ATchastd_fgsm', 'ATchastd', 'ATchastd', 'ATchastd']
    parser.add_argument('--train_AT_eps', type=float,nargs='+',default=[0.0, 0.01,0.03,0.05,0.07,0.09]) 
    ...
    log_name = os.path.join(log_path,
                            f'{args.setup}_{args.dataset}_{args.ea}_{args.model}{args.midDim}.log')
    if args.model == 'DeepCNN':
        log_name = os.path.join(log_path,
                            f'{args.setup}_{args.dataset}_{args.ea}_{args.model}{args.midDim}_{args.d1}_{args.d2}_{args.d3}.log')
    # system time
    ...
    excel_name = os.path.join(excel_path,f'{args.setup}_{args.dataset}_{args.ea}_{args.model}{args.midDim}.xlsx')
    if args.model == 'DeepCNN':
        excel_name = os.path.join(excel_path,f'{args.setup}_{args.dataset}_{args.ea}_{args.model}_{args.d1}_{args.d2}_{args.d3}.xlsx')
    ...
    for r in range(args.repeat):
        ...
        for t in range(subject_num_dict[args.dataset]):
            ...
            for k, (tra, e) in enumerate(zip(args.train, args.train_AT_eps)):
                ...
                model_path = f'/data1/cxq/model_align/{tra}/{e}/target/{args.dataset}/{args.model}{args.midDim}/{args.setup}/{args.ea}'
                if args.model == 'DeepCNN':
                    model_path = f'/data1/cxq/model_align/{tra}/{e}/target/{args.dataset}/{args.model}_{args.d1}_{args.d2}_{args.d3}/{args.setup}/{args.ea}'
                if args.FT:
