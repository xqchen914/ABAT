...
if __name__ == '__main__':
    ...
    parser.add_argument('--d1', type=int, default=20)
    parser.add_argument('--d2', type=int, default=40)
    parser.add_argument('--d3', type=int, default=80)
    
    parser.add_argument('--attacks', nargs='+',default=['FGSM', 'PGD'])#['FGSM', 'PGD']
    parser.add_argument('--epss', nargs='+',type=float,default=[0.001,0.003,0.005,0.01,0.03,0.05])#['FGSM', 'PGD']
    ...
    parser.add_argument('--train', type=str,
                        default='ATchastd')  # NT AT TL Dist TAT ATreg ATchastd NTaug
    ...
    subject_num_dict = {'MI4C': 9, 'ERN': 16, 'EPFL': 8, 'MI2014001':9,  'MI2014001-2':9,\
                        'MI2015001':12, 'MI2014004':9, 'MI2015004':9, 'weibo':10, 'weibocsp':10,\
                            'physionet':109, 'physionet2c':109}
    if args.dataset[:-1] == 'weibo':
        subject_num_dict[args.dataset] = 10
    
    sf_dict = {'MI2014001':250, 'weibo':200 }
    if args.dataset[:-1] == 'weibo':
        sf_dict[args.dataset] = 200

    # ========================model path=======================
    model_path = f'/data1/cxq/model_align/{args.train}/{args.AT_eps}/target/{args.dataset}/{args.model}{args.midDim}/{args.setup}/{args.ea}'
    if args.model == 'DeepCNN':
        model_path = f'/data1/cxq/model_align/{args.train}/{args.AT_eps}/target/{args.dataset}/{args.model}_{args.d1}_{args.d2}_{args.d3}/{args.setup}/{args.ea}'
    if args.FT:
    ...
    log_name = os.path.join(log_path,f'{args.setup}_{args.dataset}_{args.ea}_{args.model}{args.midDim}.log')
    if args.model == 'DeepCNN':
        log_name = os.path.join(log_path,f'{args.setup}_{args.dataset}_{args.ea}_{args.model}_{args.d1}_{args.d2}_{args.d3}.log')
